...
class mine:
    ...
    def mining(self):
        ...
        if self.pos == 0:
            self.game_session.money += self.efficiency

        if self.pos == 1:

            self.game_session.m1.lvluped(lvlup_by_another_mine = 1, num_of_levels=self.efficiency)

        if self.pos == 2:

            self.game_session.m2.lvluped(lvlup_by_another_mine= 1, num_of_levels=self.efficiency)
...
class Game_Session:
    def __init__(self, cost1, cost2, cost3):
        ...
        self.click_button.place(relx=0.05, rely=0.05, relwidth=0.2, relheight=0.2)
